...
public class ShowListHandler3 implements ActionListener {
	private JDialog dialog;
	private JComboBox<Integer> select;
	private JComboBox<Integer> select2;
	private List<StudentScore> ssl;
	private Font font;
	
	public ShowListHandler3(JDialog dialog, 
			JComboBox<Integer> select, JComboBox<Integer> select2, List<StudentScore> ssl) {
		this.dialog = dialog;
		this.select = select;
	...
	public void actionPerformed(ActionEvent e) {
		...
		for(StudentScore ss : ssl) {
			if(ss.getStudent().getGrade() == (Integer)select.getSelectedItem() && 
				ss.getStudent().getClassNumber() == (Integer)select2.getSelectedItem()) {
